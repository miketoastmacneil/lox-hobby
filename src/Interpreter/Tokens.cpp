...
std::ostream& operator<<(std::ostream& os, TokenType type) {
    switch (type) {
        ...
        case TokenType::FUNC:             os << "FUN"; break;
        ...
        case TokenType::LOX_EOF:         os << "LOX_EOF"; break;
    }
...
std::ostream& operator<<(std::ostream& os, const Token& token) {
        os << "Token{" 
           ...
           << ", literal: " << token.literal_.type().name();  // Print type information of literal

        if (token.literal_.type() == typeid(std::string)) {
            os << " ('" << std::any_cast<std::string>(token.literal_) << "')"; 
        } else if (token.literal_.type() == typeid(double)) {
            os << " (" << std::any_cast<double>(token.literal_) << ")";
        } // Add more cases as needed for other literal types
